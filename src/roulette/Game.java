...
public class Game {
	...
	private Bet[] myPossibleBets = { new RedBlackBet(), new EvenOddBet(),
			new ThreeInARowBet(), new PrimeBet() };
	...
	public void play(Gambler player) {
		...
		player.completeBet(myPossibleBets[whichBet].determineSuccess(myWheel, betChoice), myPossibleBets[whichBet].getOdds(), amount);
	}
	...
	private int promptForBet() {
		...
		for (int k = 0; k < myPossibleBets.length; k++) {
			System.out.println(String.format("%d) %s", (k + 1),
	...
	private String placeBet(int whichBet) {
		String result = myPossibleBets[whichBet].promptConsole();
		System.out.println();
		return result;
	}
}
